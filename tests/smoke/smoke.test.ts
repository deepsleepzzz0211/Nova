 ...
 *
 * Target: the WEIXIN gateway (https://chatapi.weixin.qq.com/openai/v1),
 * model Deepseek-v4-flash (OpenAI-compatible chat completions) — the same
 * live endpoint the e2e and baseline harnesses use, so every live tier of
 * this project speaks one contract.
 *
 ...
 *
 * Run: WEIXIN_API_KEY=... pnpm test:smoke
 */
...
const BASE_URL = 'https://chatapi.weixin.qq.com/openai/v1';
const MODEL = 'Deepseek-v4-flash';
const apiKey = process.env.WEIXIN_API_KEY;
...
function buildLoop(): {
  ...
  const resolution = resolveModel({ provider: 'weixin', model: MODEL }, catalog);
...
describe.skipIf(!hasKey)('SMOKE: full stack against weixin / Deepseek-v4-flash', () => {
  ...
  it('4. context management active — contextWindow wired from the catalog', async () => {
    ...
    // 200k context window resolved from the catalog → no truncation at this size
  ...
  it('5. web_search live (Tavily)', async () => {
    ...
    const { loop } = buildLoop();
    const turn = await loop.processUserInput(
      'Use the web_search tool to search for "typescript release notes", then name the first result title.',
    );
